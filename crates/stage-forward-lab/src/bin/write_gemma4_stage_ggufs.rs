...
impl TensorData {
    fn byte_len(&self) -> u64 {
        match self {
            ...
            TensorData::StridedRows { rows, row_bytes, .. } => rows.saturating_mul(*row_bytes),
        }
    }
}
...
fn main() -> Result<()> {
    ...
    let tail_start = args.next().and_then(|s| s.parse().ok()).unwrap_or(head_end + 1);
    ...
    let total_layers = file.inferred_layer_count().context("missing Gemma4 block count")?;
    if tail_end >= total_layers {
        bail!("tail_end={tail_end} exceeds total layer count {total_layers}");
    }

    let specs = [
        ShardSpec { role: "head", start_layer: 0, end_layer: head_end },
        ShardSpec { role: "tail", start_layer: tail_start, end_layer: tail_end },
    ];

    fs::create_dir_all(&out_dir)?;
    for spec in specs {
        let out_path =
            out_dir.join(format!("{}-{}-{}.gguf", spec.role, spec.start_layer, spec.end_layer));
        ...
        println!(
            ...
            written.metadata_u32("gemma4.attention.shared_kv_layers").unwrap_or(0),
            written.tensors.len(),
...
fn write_shard(model_path: &Path, file: &GgufFile, spec: ShardSpec, out_path: &Path) -> Result<()> {
    ...
    let full_shared_kv = file.metadata_u32("gemma4.attention.shared_kv_layers").unwrap_or(0);
    ...
    metadata.insert("gemma4.block_count".to_string(), MetadataValue::Uint32(local_layers));
...
fn collect_shard_tensors(file: &GgufFile, spec: ShardSpec) -> Result<Vec<ShardTensor>> {
    ...
    for tensor in &file.tensors {
        ...
        tensors.push(ShardTensor { source_name: tensor.name.clone(), info, data });
    }
    Ok(tensors)
}
...
fn slice_per_layer_model_proj(
    ...
    if tensor.dimensions.len() != 2 {
        bail!("per_layer_model_proj.weight must be 2D, got {:?}", tensor.dimensions);
    }
...
fn slice_per_layer_token_embd(
    ...
    if tensor.dimensions.len() != 2 {
        bail!("per_layer_token_embd.weight must be 2D, got {:?}", tensor.dimensions);
    }
...
fn array_element_type(values: &[MetadataValue]) -> Result<u32> {
    let first = values.first().context("cannot write empty GGUF metadata array")?;
...
fn write_tensor_data<R: Read + Seek, W: Write>(
    ...
    match data {
        TensorData::Copy { source_file_offset, byte_len } => {
...
fn checked_mul(lhs: u64, rhs: u64, context: &'static str) -> Result<u64> {
    lhs.checked_mul(rhs).with_context(|| format!("{context} overflow: {lhs} * {rhs}"))
}

fn checked_add(lhs: u64, rhs: u64, context: &'static str) -> Result<u64> {
    lhs.checked_add(rhs).with_context(|| format!("{context} overflow: {lhs} + {rhs}"))
}
