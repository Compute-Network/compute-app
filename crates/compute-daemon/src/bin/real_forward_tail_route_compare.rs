...
async fn main() -> Result<()> {
    ...
    println!("=== Real-Forward Tail Route Compare ===");
    ...
    for case in validation_prompt_cases(suite_mode) {
        ...
        let request_id = format!("tail-route-{}", case.name);
        ...
        let mut reference_tail_provider = build_real_forward_provider(&reference_plan);
        let mut candidate_tail_provider = build_real_forward_provider(&candidate_plan);
        let direct_tail_backend = load_backend(&tail_spec, &model_name, false, true, None)?;

        reference_head_provider.load_shard(&head_config)?;
        candidate_head_provider.load_shard(&head_config)?;
        reference_tail_provider.load_shard(&tail_config)?;
        candidate_tail_provider.load_shard(&tail_config)?;
        ...
        let direct_ref_request_id = format!("{request_id}-direct-ref");
        let direct_candidate_request_id = format!("{request_id}-direct-candidate");
        let provider_ref_request_id = format!("{request_id}-provider-ref");
        let provider_candidate_request_id = format!("{request_id}-provider-candidate");

        let direct_tail_from_ref = direct_tail_backend
            .continue_forward(retag_stage_tensor(&reference_head_output, &direct_ref_request_id))?;
        let direct_tail_from_candidate = direct_tail_backend.continue_forward(
            retag_stage_tensor(&candidate_head_output, &direct_candidate_request_id),
        )?;
        direct_tail_backend.clear_decode_session(&direct_ref_request_id);
        direct_tail_backend.clear_decode_session(&direct_candidate_request_id);

        let provider_tail_from_ref = reference_tail_provider.continue_forward(
            retag_stage_tensor(&reference_head_output, &provider_ref_request_id),
        )?;
        let provider_tail_from_candidate = candidate_tail_provider.continue_forward(
            retag_stage_tensor(&candidate_head_output, &provider_candidate_request_id),
        )?;
        ...
        println!(
            "direct tail(ref<-ref) vs provider tail(ref)      : {}",
            tensor_max_abs(&direct_tail_from_ref, &provider_tail_from_ref)?
        );
        println!(
            "direct tail(ref<-cand) vs provider tail(cand)    : {}",
            tensor_max_abs(&direct_tail_from_candidate, &provider_tail_from_candidate)?
        );
        println!(
            "direct tail(ref<-ref) vs direct tail(ref<-cand)  : {}",
            tensor_max_abs(&direct_tail_from_ref, &direct_tail_from_candidate)?
        );
        println!(
            "provider tail(ref) vs provider tail(cand)        : {}",
            tensor_max_abs(&provider_tail_from_ref, &provider_tail_from_candidate)?
        );
        println!();

        reference_head_provider.clear_decode_session(&request_id);
        candidate_head_provider.clear_decode_session(&request_id);
        reference_tail_provider.clear_decode_session(&provider_ref_request_id);
        candidate_tail_provider.clear_decode_session(&provider_candidate_request_id);
        reference_head_provider.unload()?;
        candidate_head_provider.unload()?;
        reference_tail_provider.unload()?;
        candidate_tail_provider.unload()?;
    }
