...
fn diff_stats(lhs: &[f32], rhs: &[f32]) -> Result<(f64, f32, f32, f32)> {
    ...
    Ok((sum_abs / n, max_abs, (lhs_sq / n).sqrt() as f32, (rhs_sq / n).sqrt() as f32))
}

fn main() -> Result<()> {
    let args: Vec<String> = std::env::args().collect();
    if args.len() < 5 {
        bail!("{}", usage(args.first().map(String::as_str).unwrap_or("compare_real_shards")));
    }
    ...
    println!("head_hidden_bytes_equal={}", full_hidden.bytes == real_hidden.bytes);
    ...
    println!("tail_sample_match={}", full_sample.token_id == real_sample.token_id);
